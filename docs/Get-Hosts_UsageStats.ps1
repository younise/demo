...
Get-VMhost $vmhost | % {
$VMhost =$_
$stat =get-stat -entity $vmhost -maxsamples 1 -intervalmins 5
$cpuusage5mins =($stat | ? { $_.metricID -like "*cpu.usage.average*" } ).Value
$netusage5mins =($stat | ? { $_.metricID -like "*net.usage.average*" } ).Value
$cpureadysummation5mins =Get-Stat -Entity $vmhost -Stat cpu.ready.summation -MaxSamples 1 -IntervalMins 5   | select Value
...
$ready = [math]::round((($cpureadysummation5mins).value / 300000 / ($vmhost | get-vm | ? { $_.Powerstate -eq "poweredon" }| measure-object -Sum NumCPU).Sum *100 ) ,2)
